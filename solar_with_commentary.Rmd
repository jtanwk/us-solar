...
gen %>%
  ...
  labs(title = "Hawaii generates over 50x more solar energy per capita than natl. median",
       ...
       x = "", 
       y = "Solar Energy per Capita Generated, Relative to Median State",
       caption = "Data source: SEDS (U.S. Energy Information Administration)") +
  annotate("segment", 
           x = "HI", xend = "HI", 
           y = 1.2, yend = 3.2, 
...
panels_by_pop %>%
  ...
  facet_grid(. ~ region,
             scales = "free_x",
             space = "free_x") +
  scale_fill_manual(values = c(theme_bg_gray, theme_purple)) +
...
county_map %>%
  ...
  labs(title = "Large panels-per-capita in the Northeast, despite low annual solar radiation",
       subtitle = "While the Northeast region receives the least solar irradiance in the US, it also has states with \ndisproportionately high solar panel installation rates.",
